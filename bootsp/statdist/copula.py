...
class CopulaBase(MultivariateDistribution):
    ...
    def __init__(self, dimension, dimkeys=None, parameters=None):
        """
        Args:
            dimension (int): The dimension of the distribution
            dimkeys (list): A list of the names of each distribution, if not
                passed in will default to integers 0,1,..,n
            parameters (list): A list of parameters for the distribution
        """
        MultivariateDistribution.__init__(self, dimension, dimkeys, parameters,
...
class CopulaWithMarginals(MultivariateDistribution):
    ...
    def __init__(self, copula, marginals, dimkeys=None):
        """
        Args:
            copula (CopulaBase): The instantiated copula encoding the
                dependence between variables
            marginals (dict|list): A list of marginal distributions, each
                a univariate distribution, or a dictionary mapping elements
                of dimkeys to a corresponding marginal distribution
            dimkeys (list): Optionally, a list of the names of each of the
                dimensions
        """
        ...
        # We construct this distributions parameters from the parameters for
        # the constituent distributions with each distribution's name prepended
        params = []
        for param in copula.params:
            params.append(Parameter(copula.name + '_' + param.name,
                                    param.values, param.bounds, param.kind))

        for i, marginal in enumerate(self.marginals):
            for param in marginal.params:
                param_name = str(i) + '_' + marginal.name + '_' + param.name
                params.append(Parameter(param_name, param.values,
                                        param.bounds, param.kind))
    ...
    def __init__(self, R, dimkeys=None):
        ...
        for i in range(ndim):
            for j in range(i, ndim):
                value = R[i,j]
                R_params.append(Parameter('R_{}{}'.format(i, j), value))

        CopulaBase.__init__(self, ndim, dimkeys, R_params)
        self.cor_matrix = R
...
class StudentCopula(CopulaBase):
    ...
    def __init__(self, cov, df=5, dimkeys=None):
        ...
        for i in range(ndim):
            for j in range(i, ndim):
                value = cov[i,j]
                cov_params.append(Parameter('cov_{}{}'.format(i, j), value))
        df_param = Parameter('df', df, (0, None))

        CopulaBase.__init__(self, ndim, dimkeys, cov_params + [df_param])
...
class FrankCopula(CopulaBase):
    ...
    def __init__(self, theta, dimkeys=None):
        ...
        # The frank Copula is defined on [-inf,0)U(0,inf] which cannot be
        # encoded in our Parameter scheme
        param = Parameter('theta', theta)

        CopulaBase.__init__(self, 2, dimkeys, [param])
...
class ClaytonCopula(CopulaBase):
    ...
    def __init__(self, theta, dimkeys):
        ...
        """
        self.theta = theta
        param = Parameter('theta', theta, (epsilon, None))
        CopulaBase.__init__(self, 2, dimkeys, [param])
...
class GumbelCopula(CopulaBase):
    ...
    def __init__(self, theta, dimkeys=None):
        ...
        """
        self.theta = theta
        param = Parameter('theta', theta, (1, None))
...
class EmpiricalCopula(CopulaBase):
    def __init__(self, input_data, dimkeys=None, marginals=None):
        ...
        if marginals is None:
            marginals = [UnivariateEmpiricalDistribution(input_data[i])
                         for i in range(ndim)]
