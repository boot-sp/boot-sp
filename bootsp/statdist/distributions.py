...
class MultiNormalDistribution(MultivariateDistribution):
    ...
    def __init__(self, mean, cov, dimkeys=None):
        ...
        # We construct the Parameter objects for the mean and covariance matrix
        # Since Parameter objects are 1D, we will have 1 for each component
        # of the mean vector and for each component of the upper triangle of
        # the covariance matrix.
        mean_params = []
        for i, xi in enumerate(mean_params):
            mean_params.append(Parameter('mean_{}'.format(i), value))
        ...
        params = mean_params + cov_params
        MultivariateDistribution.__init__(self, ndim, dimkeys, params)
    ...
    def pdf(self, *xs):
        ...
        Examples:
            ...
            >>> d = MultiNormalDistribution.fit(xs, dimkeys=['A', 'B'])
            >>> d.pdf({'A': 1, 'B': 0})
...
class MultiStudentDistribution(MultivariateDistribution):
    ...
    def __init__(self, mean, cov, df=5):
        ...
        for i in range(ndim):
            for j in range(i, ndim):
                value = cov[i,j]
                param = Parameter('cov_{}{}'.format(i, j), value, (0, None))
                cov_params.append(param)

        params = mean_params + cov_params + [Parameter('df', df, (epsilon, None))]

        MultivariateDistribution.__init__(self, ndim, params)
...
class UnivariateGaussianKernelDistribution(UnivariateDistribution):
    ...
    def __init__(self, input_data, bw_method=None, dom_std=1):
        ...
        self.kernel = gaussian_kde(input_data, bw_method)
...
class MultivariateUniformDistribution(MultivariateDistribution):
    ...
    def __init__(self, bounds, dimkeys=None):
        ...
        for i, bound in enumerate(bounds):
            params.append(Parameter('a_{}'.format(i), bound[0]))
            params.append(Parameter('b_{}'.format(i), bound[0]))

        MultivariateDistribution.__init__(self,dimension, dimkeys, params)
